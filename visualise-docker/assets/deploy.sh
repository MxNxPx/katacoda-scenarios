...
cd /root
wget $(curl -s https://api.github.com/repos/vmware-tanzu/carvel-ytt/releases/latest | egrep "browser_download_url.*linux.*64" | cut -d '"' -f 4) && mv ytt*linux*64 /usr/local/bin/ytt && chmod 755 /usr/local/bin/ytt
